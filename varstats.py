# This code computes group t-values and mathed-pairs t-values (thresholded) for variance pre- and post- retroicor.
...
for subj in sessions.keys():
    for sess in sessions[subj]:
        prefiles.append(basepath+'/'+subj+'/'+sess+'/fepi/fepi_pipeline_noRet_slicetimer_mcflirt_brainExtractAFNI_ssmooth_3dFourier_var_mni152.nii.gz')
        postfiles.append(basepath+'/'+subj+'/'+sess+'/fepi/fepi_pipeline_slicetimer_mcflirt_brainExtractAFNI_ssmooth_3dFourier_retroicor_var_mni152.nii.gz')
        
prepostmatchedpairst(prefiles,postfiles,ofile)

groupnetwork(prefiles,basepath+'/var_pregroupnetwork.nii.gz')
groupnetwork(postfiles,basepath+'/var_postgroupnetwork.nii.gz')
