...
ofileresp=basepath+'/Presp_prepostmatchedpairst.nii.gz'
ofilecard=basepath+'/Pcard_prepostmatchedpairst.nii.gz'
...
# first transform psd files to mni
for subj in subjects:
    for sess in subj.sessions:
        psdfile=basepath+'/'+subj.ID+'/'+sess.ID+'/fepi/fepi_pipeline_noRet_slicetimer_mcflirt_brainExtractAFNI_ssmooth_3dFourier_Presp.nii.gz'
        ...
        psdfile=basepath+'/'+subj.ID+'/'+sess.ID+'/fepi/fepi_pipeline_noRet_slicetimer_mcflirt_brainExtractAFNI_ssmooth_3dFourier_Pcard.nii.gz'
        p=subprocess.Popen(['flirt', '-in', psdfile,\
                            '-applyxfm', '-init', sess.runs[0].data.func2mni,\
                            '-out', fileutils.removext(psdfile)+'_'+'mni152',\
                            '-ref', mni152])
        ...
        psdfile=basepath+'/'+subj.ID+'/'+sess.ID+'/fepi/fepi_pipeline_slicetimer_mcflirt_brainExtractAFNI_ssmooth_3dFourier_retroicor_Presp.nii.gz'
        p=subprocess.Popen(['flirt', '-in', psdfile,\
                            '-applyxfm', '-init', sess.runs[0].data.func2mni,\
                            '-out', fileutils.removext(psdfile)+'_'+'mni152',\
                            '-ref', mni152])
        ...
        psdfile=basepath+'/'+subj.ID+'/'+sess.ID+'/fepi/fepi_pipeline_slicetimer_mcflirt_brainExtractAFNI_ssmooth_3dFourier_retroicor_Pcard.nii.gz'
        p=subprocess.Popen(['flirt', '-in', psdfile,\
                            '-applyxfm', '-init', sess.runs[0].data.func2mni,\
                            '-out', fileutils.removext(psdfile)+'_'+'mni152',\
                            '-ref', mni152])
...
for subj in sessions.keys():
    for sess in sessions[subj]:
        prefiles.append(basepath+'/'+subj+'/'+sess+'/fepi/fepi_pipeline_noRet_slicetimer_mcflirt_brainExtractAFNI_ssmooth_3dFourier_Presp_mni152.nii.gz')
        postfiles.append(basepath+'/'+subj+'/'+sess+'/fepi/fepi_pipeline_slicetimer_mcflirt_brainExtractAFNI_ssmooth_3dFourier_retroicor_Presp_mni152.nii.gz')
        
prepostmatchedpairst(prefiles,postfiles,ofileresp)

groupnetwork(prefiles,basepath+'/Presp_pregroupnetwork.nii.gz')
groupnetwork(postfiles,basepath+'/Presp_postgroupnetwork.nii.gz')

# then do card
for subj in sessions.keys():
    for sess in sessions[subj]:
        prefiles.append(basepath+'/'+subj+'/'+sess+'/fepi/fepi_pipeline_noRet_slicetimer_mcflirt_brainExtractAFNI_ssmooth_3dFourier_Pcard_mni152.nii.gz')
        postfiles.append(basepath+'/'+subj+'/'+sess+'/fepi/fepi_pipeline_slicetimer_mcflirt_brainExtractAFNI_ssmooth_3dFourier_retroicor_Pcard_mni152.nii.gz')
        
prepostmatchedpairst(prefiles,postfiles,ofilecard)

groupnetwork(prefiles,basepath+'/Pcard_pregroupnetwork.nii.gz')
groupnetwork(postfiles,basepath+'/Pcard_postgroupnetwork.nii.gz')
