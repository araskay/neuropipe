...
% in this version, all powers are normalized to the nophyscor power on a
% per subject basis (note that in physspectra_physcor.m normalization is to
% ret.
...
fcardout=fopen('/global/home/hpc3820/data/healthyvolunteer/processed/physcor/cardPowerSpectra_withmotreg.csv','w');
frespout=fopen('/global/home/hpc3820/data/healthyvolunteer/processed/physcor/respPowerSpectra_withmotreg.csv','w');
flowout=fopen('/global/home/hpc3820/data/healthyvolunteer/processed/physcor/lowPowerSpectra_withmotreg.csv','w');
...
fprintf(fcardout,'Subject,ret,comp,retcomp,compret,globalsigreg,csfwmreg,nophyscor\n');
fprintf(frespout,'Subject,ret,comp,retcomp,compret,globalsigreg,csfwmreg,nophyscor\n');
fprintf(flowout,'Subject,ret,comp,retcomp,compret,globalsigreg,csfwmreg,nophyscor\n');
...
for i=1:n
    ...
    retfile=strcat(basepath,subject,'/fepi/fepi_ret_withmotreg_slicetimer_mcflirt_fsl_motion_outliers_motreg_brainExtractAFNI_ssmooth_3dFourier_retroicor_meants_net.txt');
    compfile=strcat(basepath,subject,'/fepi/fepi_comp_withmotreg_slicetimer_mcflirt_fsl_motion_outliers_motreg_brainExtractAFNI_ssmooth_3dFourier_tcompcor_meants_net.txt');
    retcompfile=strcat(basepath,subject,'/fepi/fepi_retcomp_withmotreg_slicetimer_mcflirt_fsl_motion_outliers_motreg_brainExtractAFNI_ssmooth_3dFourier_retroicor_tcompcor_meants_net.txt');
    compretfile=strcat(basepath,subject,'/fepi/fepi_compret_withmotreg_slicetimer_mcflirt_fsl_motion_outliers_motreg_brainExtractAFNI_ssmooth_3dFourier_tcompcor_retroicor_meants_net.txt');
    nophyscorfile=strcat(basepath,subject,'/fepi/fepi_nophyscor_withmotreg_slicetimer_mcflirt_fsl_motion_outliers_motreg_brainExtractAFNI_ssmooth_3dFourier_meants_net.txt');
    globalsigregfile=strcat(basepath,subject,'/fepi/fepi_globalsigreg_withmotreg_slicetimer_mcflirt_fsl_motion_outliers_motreg_brainExtractAFNI_ssmooth_3dFourier_globalsigreg_meants_net.txt');
    csfwmregfile=strcat(basepath,subject,'/fepi/fepi_csfwmreg_withmotreg_slicetimer_mcflirt_fsl_motion_outliers_motreg_brainExtractAFNI_ssmooth_3dFourier_globalsigreg_meants_net.txt');
    ...
    meants_globalsigreg_fid=fopen(globalsigregfile);
    meants_globalsigreg=fscanf(meants_globalsigreg_fid,'%f');

    meants_csfwmreg_fid=fopen(csfwmregfile);
    meants_csfwmreg=fscanf(meants_csfwmreg_fid,'%f');    
    ...
    meants_globalsigreg=meants_globalsigreg(ndiscard+1:end);
    meants_csfwmreg=meants_csfwmreg(ndiscard+1:end);
    ...
    subplot(7,1,5)
    plot(f,2*abs(Fmeants_globalsigreg(1:l/2+1))/l)
    ...
    subplot(7,1,6)
    plot(f,2*abs(Fmeants_csfwmreg(1:l/2+1))/l)
    title('CSF/WM Regression');
    %xlabel('Frequency (Hz)')
    ylabel('PSD (I^2/Hz)')
    ylim([0, 0.05])    
    ...
    fprintf(fcardout,'%f,%f,%f,%f,%f,%f,%f\n',card);

    resp=[respRelativePower_ret,respRelativePower_comp,respRelativePower_retcomp,respRelativePower_compret,respRelativePower_globalsigreg,respRelativePower_csfwmreg,respRelativePower_nophyscor];
    fprintf(frespout,'%s,',subject);
    fprintf(frespout,'%f,%f,%f,%f,%f,%f,%f\n',resp);
    %lowRelativePower_lpf=sum(abs(Fmeants_lpf(flow_min_ind:flow_max_ind)).^2)/sum(abs(Fmeants_ret).^2);
    
    low=[lowRelativePower_ret,lowRelativePower_comp,lowRelativePower_retcomp,lowRelativePower_compret,lowRelativePower_globalsigreg,lowRelativePower_csfwmreg,lowRelativePower_nophyscor];
    fprintf(flowout,'%s,',subject);
    fprintf(flowout,'%f,%f,%f,%f,%f,%f,%f\n',low);
